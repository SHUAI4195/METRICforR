...
author: "Guillermo Federico Olmedo, Fernando Fuentes-Peñailillo, Daniel de la Fuente-Saiz and David Fonseca-Luengo"
date: "`r Sys.Date()`"
...
There are two version of this vignette: Simple and advanced version. 
In the first one the procedure is simpler but most of the parameters are selected automatically by the package. 
...
Also, on the simple procedure we are going to assume flat terrain, but in the advanced procedure we are going to use a digital elevation model (DEM). However, is possible to use DEM in the simple procedure, changing the parameter `flat` to `FALSE` and providing a DEM.
...
We estimate $Rn$, $G$ and $H$ for each pixel using a *Landsat satellite scene* and meteorological weather station data. 
Then we estimate $LE$ fluxes using the previous equation, and after that, the instantaneous evapotranspiration values were calculated as follows:
...
- Raw Landsat 7-8 satellite image (original .TIF data from glovis USGS).
- Weather Station data (.CSV file).
- Area-of-interest (AOI) (if we don´t define an AOI, we will estimate crop evapotranspiration for the entire landsat scene).
...
```
Then, we load weather station data. For that, we use `read.WSdata` function. 
This command transforms our .csv file into a `waterWeatherStation` object. 
...
Usually, using `water`, we can use the  function called `loadImage` to load a Landsat image from `.TIF files` downloaded directly
from [Earth Explorer](http://earthexplorer.usgs.gov/) or [GLOVIS](https://glovis.usgs.gov/). 
...
Finally we are going to create The Digital Elevation Model (DEM) for the specific satellite image we are processing. 
Thus, we needed a image-specifc grid files downloadable from [Earth Explorer](http://earthexplorer.usgs.gov/). 
Therefore, check wich grid files we are going to need considering the satellite scene location and the AOI polygon, using:
